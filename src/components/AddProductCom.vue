  ...
  <div>
    <form @submit.prevent="addProduct()">
      ...
      <p v-if="uploadValue > 0 && uploadValue < 100">
        รอซักครู่ กำลังอัปโหลดไฟล์ {{ uploadValue }} %
      </p>
      ...
      <p>
        <label>รายละเอียดสินค้า: </label><br /><textarea
          style="width: 100%"
      ...
      <hr />
      <p><input type="submit" value="บันทึกรายการสินค้า" /></p>
    </form>
  </div>
</template>
...
export default {
  ...
  methods: {
    addProduct() {
      const refAdd = db.collection("products");
      refAdd
        .add(this.formProduct)
        .then(() => {
          console.log("ok");
          alert("Ok saved");
          this.formProduct = {
